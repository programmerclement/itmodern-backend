...
export function verificationEmail({ firstName, verifyUrl }) {
  return {
...
export function passwordResetEmail({ firstName, resetUrl }) {
  return {
    subject: 'Reset your ITMODERN password',
    html: emailLayout({
      ...
      bodyHtml: `
        ...
        <p style="margin-top:24px;color:#64748b;font-size:12px;">This link expires in 1 hour. If you didn't request this, you can safely ignore this email — your password won't change.</p>
      `,
    }),
  };
}

export function welcomeEmail({ firstName, shopUrl }) {
  return {
...
export function orderConfirmationEmail({ firstName, order, orderUrl }) {
...
export function paymentReceivedEmail({ firstName, order, payment, orderUrl }) {
  return {
...
export function quotationReadyEmail({ firstName, quotation, quotationUrl }) {
